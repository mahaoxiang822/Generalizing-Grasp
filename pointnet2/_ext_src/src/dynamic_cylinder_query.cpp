...
#include "dynamic_cylinder_query.h"
#include "utils.h"
...
at::Tensor dynamic_cylinder_query(at::Tensor new_xyz, at::Tensor xyz, at::Tensor rot, at::Tensor radius, const float hmin, const float hmax,
  ...
  if (new_xyz.type().is_cuda()) {
    dynamic_query_cylinder_point_kernel_wrapper(xyz.size(0), xyz.size(1), new_xyz.size(1),
                                    radius.data<float>(), hmin, hmax, nsample, new_xyz.data<float>(),
