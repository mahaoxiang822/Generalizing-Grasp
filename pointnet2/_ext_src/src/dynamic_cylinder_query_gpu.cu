...
__global__ void dynamic_query_cylinder_point_kernel(int b, int n, int m, const float *__restrict__ radius, float hmin, float hmax,
                                        int nsample,
...
void dynamic_query_cylinder_point_kernel_wrapper(int b, int n, int m, const float *radius, float hmin, float hmax,
                                     int nsample, const float *new_xyz,
                                     const float *xyz, const float *rot, int *idx) {
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  dynamic_query_cylinder_point_kernel<<<b, opt_n_threads(m), 0, stream>>>(
